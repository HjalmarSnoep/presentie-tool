...
<?php
	...
	// set Cookie!
	// This is NOT used for creating the fingerprint, that would be too breakable.
	// it can be used later for heuristic client recognition AI.
	// but we shouldn't trust it.
	
	/* get the mac adress, only works if client on same LAN!
		...
		on linux end<?php
		echo exec('getmac'); or system('getmac') on Windows server!
		Also only same LAN.!
	*/
	...
	$ffp="";
	if(isset($_REQUEST["ffp"])) $avf=$_REQUEST["ffp"];
	$nva="";
	if(isset($_REQUEST["nva"])) $avf=$_REQUEST["nva"];
	$nvc="";
	if(isset($_REQUEST["nvc"])) $avf=$_REQUEST["nvc"];
	$nvp="";
	if(isset($_REQUEST["nvp"])) $avf=$_REQUEST["nvp"];
	$ffp="";
...
//	echo strlen($fingerprint);
	...
	if($student_valid)
	{
		...
		{
			$fp_from_file=file_get_contents ($fingerprint_file); // associate $unr with this fingerprint.
			// check fingerprint against calculated one.
			if($fingerprint!=$fp_from_file)
			{
				echo "ERROR 1: FINGERPRINT MISMATCH, did you use the same browser? <br>";
				echo "Twee opties: <br>";
				echo "<ol>";
				//send_reset_request.php?unr=19658
				echo "<li>Beantwoord <a href='send_reset_request.php?unr=".$unr."'>deze mail</a> (1xper 24 uur)</li>";
				echo "<li>Meldt je bij Snoep om een nieuwe fingerprint te claimen.</li>";
				echo "</ol>";
				setStat("different fingerprint stored for (".$unr.") at ");
				exit(); // we DO NOT go on!
			}
		}else{
			//make fingerprint
			file_put_contents ($fingerprint_file,$fingerprint); // associate $unr with this fingerprint.
			echo "new FINGERPRINT recorded!<br>";
			setStat("new fingerprint for unr (".$unr.")");
		}
		*/
	}else{
		if(!isset($_REQUEST["date"]))
		{
			echo "Error: dat is geen valide studenten nummer.";
			exit();
		}
	}
...
setInterval(loop,1000,true); // every minute.
if(iamateacher)
{
